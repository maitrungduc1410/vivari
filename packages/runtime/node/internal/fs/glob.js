// DERIVED from Node.js — lib/internal/fs/glob.js
// Upstream: https://github.com/nodejs/node/blob/v24.18.0/lib/internal/fs/glob.js
...
// process.binding('natives')). Backs fs.glob / fs.globSync / path.matchesGlob.
//
// NOT verbatim: one added export, marked `VIVARI DELTA` (matchGlobPattern), which
// v24's lib/path.js requires from here. See the comment at that hunk.
...
// VIVARI DELTA: v24 moved path.matchesGlob's matcher out of lib/path.js and into
// this module as `matchGlobPattern`; the v22 body below does not export it, so our
// vendored v24 lib/path.js (`require('internal/fs/glob').matchGlobPattern`) would
// get undefined and die as "lazyMatchGlobPattern(...) is not a function". The body
// is v22's lib/path.js `glob()` helper verbatim — same upstream code, just from the
// file it lived in one major earlier.
