// DERIVED from Node.js — lib/internal/fs/rimraf.js (itself a fork of the npm
// `rimraf` module). Upstream: https://github.com/nodejs/node/blob/v24.18.0/lib/internal/fs/rimraf.js
...
// process.binding('natives')). Our vendored v24 lib/fs.js uses only the async
// `{ rimraf }` export (fs.rm) — fs.rmSync goes straight to binding.rmSync.
//
// NOT verbatim. Three hunks differ from upstream, each marked `VIVARI DELTA`:
// upstream lists a directory's children with a 'buffer' encoding and builds the
// child paths with Buffer.concat, but our internalBinding('fs').readdir IGNORES
// the encoding argument and always returns strings — so the verbatim body throws
// `ERR_INVALID_ARG_TYPE: The "list[1]" argument must be of type Buffer` on the
// first NON-EMPTY directory, i.e. exactly what fs.rm(dir, {recursive:true}) is
// for. The deltas list and join as strings instead. A Buffer path handed in by a
// caller still works (template-string coercion yields the utf8 path).
// If bindings/fs.js ever honours the readdir encoding, revert to verbatim.
...
function _rmchildren(path, options, callback) {
  // VIVARI DELTA: list + join as strings (upstream: 'buffer' encoding + Buffer.concat).
  const pathStr = `${path}`;

  readdir(pathStr, (err, files) => {
    if (err)
      return callback(err);
    ...
    const childPathPrefix = pathStr + sep;

    for (let i = 0; i < files.length; i++) {
      const childPath = childPathPrefix + files[i];
...
function _rmdirSync(path, options, originalErr) {
  try {
    ...
    if (notEmptyErrorCodes.has(err.code)) {
      ...
      // VIVARI DELTA: list + join as strings (see _rmchildren).
      const pathStr = `${path}`;

      ArrayPrototypeForEach(readdirSync(pathStr), (child) => {
        const childPath = pathStr + sep + child;
