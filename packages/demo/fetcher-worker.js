...
// Host alias — the analog of Docker's `host.docker.internal`. In-VM code can reach
// a service running on the HOST machine (the machine running the browser, e.g. a
// real dev server on localhost:3000) by addressing it as
// `http://host.opencontainer.internal:<port>/…`. We map the alias to the studio's
// OWN hostname (the fetcher runs in the browser, so self.location.hostname IS the
// host), preserving scheme/port/path. `host.docker.internal` is accepted too so
...
const HOST_ALIASES = new Set(["host.opencontainer.internal", "host.docker.internal"]);

function rewrite(url) {
  try {
    const u = new URL(url);
    if (HOST_ALIASES.has(u.hostname)) {
