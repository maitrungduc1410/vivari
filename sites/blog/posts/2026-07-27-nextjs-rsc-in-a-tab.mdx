...
like Node does. A handful of modules — `dns/promises`, `stream/web`, an
`inspector` stub, `module.findSourceMap`, and the complete `Console` method
surface that `@edge-runtime/primitives` binds — had to exist.
...
The patches install **once at boot** — after the runtime's own timer globals are
in place and before any framework code loads, so React captures the wrapped
primitives rather than the originals — and only on the polyfill path. When the
host provides real `async_hooks`, none of this is active.
...
cut, no specification to implement completely, no primitive to respect —
V8's PromiseHook simply is not there. So the only honest move was to narrow the
